...
SELECT 'Influenza', 'inclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
--Put concept_ids here
4266367	--	6142004	Condition	Influenza	SNOMED
    )
;
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Influenza', 'not_mapped',
       c.*
FROM @vocabulary_database_schema.concept c
--Mask to detect uncovered concepts
WHERE c.concept_name ~* 'influenza| flu$|flu |Orthomyxov|Ortomyxov'
--Masks to exclude
    AND c.concept_name !~* 'Haemophilus|Influenza season|Hemophilus|Parainfluenza|vaccine|not detected|not isolated|vaccination|advise|Exposure|immunisation|contact|immunization|antibody level|detection assay|antigen level|Advice|consultation|Educated|serology'

    AND c.domain_id IN ('Condition', 'Observation' /*,'Measurement'*/)

    AND c.concept_class_id NOT IN ('Substance', 'Organism', 'LOINC Component')

    AND c.vocabulary_id NOT IN ('MedDRA', 'SNOMED Veterinary', 'MeSH', 'CIEL', 'OXMIS')
    ...
    AND NOT EXISTS (
        ...
        WHERE ca1.ancestor_concept_id IN (
            SELECT concept_id
            FROM @target_database_schema.concept_phenotypes
            WHERE phenotype IN ('Influenza', 'Influenza (complicated)')
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Influenza', 'exclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
--Put concept_ids here
45770619,	--	932221000000103	Condition	At risk of influenza related complication	SNOMED
764960,	--	921000124107	Condition	Influenza A virus inconclusive	SNOMED
45757528,	--	181000124108	Condition	Influenza A virus present	SNOMED
4262075,	--	441345003	Condition	Influenza B virus present	SNOMED
4319159,	--	95891005	Condition	Influenza-like illness	SNOMED
4153160,	--	315642008	Condition	Influenza-like symptoms	SNOMED
4042202,	--	230188005	Condition	Post-influenza encephalitis	SNOMED
44802950,	--	506751000000104	Observation	Possible influenza due to Influenza A virus subtype H1N1	SNOMED
44813193	--	505251000000100	Observation	Suspected influenza A virus subtype H1N1 infection	SNOMED
    )
;
