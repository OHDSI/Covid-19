...
SELECT 'Liver disease', 'inclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
194984	--	235856003	Condition	Disease of liver	SNOMED
    )
;
...
WHERE (
        ...
        (c.concept_name ~* '(liver).+( disease)|(disease).+( liver)|(liver).+( disorder)|(liver).+( lesion)|(disorder).+( liver)|hepatopathy'

        --Masks to exclude
         AND c.concept_name !~* 'lymphoproliferative'
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Liver disease', 'exclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
