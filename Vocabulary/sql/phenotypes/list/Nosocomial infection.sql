...
WHERE phenotype = 'Nosocomial infection'
;
...
SELECT 'Nosocomial infection', 'inclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
--Put concept_ids here
4256893	--	408678008	Condition	Healthcare associated infectious disease	SNOMED
    )
;
...
WHERE (
        ...
        (c.concept_name ~* 'Nosocomial|(Hospital).*(acquired)'

        --Masks to exclude
        AND c.concept_name !~* 'Acinetobacter|pressure ulcer'
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Nosocomial infection', 'exclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
--Put concept_ids here
763165	--	436751000124107	Condition	Sepsis associated with internal vascular access	SNOMED
    )
;
