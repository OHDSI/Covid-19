...
WHERE phenotype = 'Cardiovascular disease'
;
...
SELECT 'Cardiovascular disease', 'inclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
134057	--	49601007	Condition	Disorder of cardiovascular system	SNOMED
    )
;
...
WHERE (
        ...
        (c.concept_name ~* '(cardiovascular).*(disease)|(disease).*(cardiovascular)|(lesion).*(heart)|(heart).*(disease)|(disease).*(heart)'

        --Masks to exclude
         AND c.concept_name !~* 'monitoring|assessment|risk|review|phenx|without|screen|observation|family|lymph|suspect|FH|prevent|exc|oma'
