...
WHERE phenotype = 'Other metabolic disorder'
;
...
SELECT 'Other metabolic disorder', 'inclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
436670	--	75934005	Condition	Metabolic disease	SNOMED
    )
;
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Other metabolic disorder',
       'not_mapped',
       c.*
FROM @vocabulary_database_schema.concept c
...
WHERE (
        ...
        (c.concept_name ~* '(metabolic).*(disease|syndrom|disorder)'

        --Masks to exclude
         AND c.concept_name !~* 'family|screen|classification|drug|fh|abortion'
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Other metabolic disorder', 'exclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
    4034964	--	237620003	Condition	Abnormal metabolic state in diabetes mellitus	SNOMED
