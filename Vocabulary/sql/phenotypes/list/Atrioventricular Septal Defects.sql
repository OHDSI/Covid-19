...
WHERE phenotype = 'Atrioventricular Septal Defects'
;
...
SELECT 'Atrioventricular Septal Defects', 'inclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
--Put concept_ids here
42537639 	--	737156006	Condition	Congenital anomaly of atrioventricular septum	SNOMED
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Atrioventricular Septal Defects',
       'not_mapped',
       c.*
FROM @vocabulary_database_schema.concept c
...
WHERE (
        ...
        (c.concept_name ~* 'AVSD|(Atrioventr).*(sept).*(def)|(def).*(atrioventr).*(sept)'
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Atrioventricular Septal Defects', 'exclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
--Put concept_ids here
