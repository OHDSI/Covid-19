...
WHERE phenotype = 'Cerebrovascular disease'
;
...
WHERE (
        ...
        (c.concept_name ~* 'CVD|(cerebrovascular).*( disease)|(disease).*( cerebrovascular)|(cerebrovascular).*( lesion)'

        --Masks to exclude
         AND c.concept_name !~* 'declined|co-morbid|history'
...
INSERT INTO @target_database_schema.concept_phenotypes
SELECT 'Cerebrovascular disease', 'exclusion', c.*
FROM @vocabulary_database_schema.concept c
WHERE c.concept_id IN (
